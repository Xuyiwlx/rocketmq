...
public class CommitLog {
    ...
    public void recoverNormally(long maxPhyOffsetOfConsumeQueue) {
        ...
        if (!mappedFiles.isEmpty()) {
            ...
            // Broker 正常停止再重启时,从倒数第三个文件开始进行恢复
            // 如果不足三个,则从第一个文件开始恢复
            ...
            // CommitLog 文件已确认的物理偏移量等于mappedFile.getFileFromOffset
            // 加上 mappedFileOffset
            long processOffset = mappedFile.getFileFromOffset();
            // 当前文件已校验通过的offset
            long mappedFileOffset = 0;
            while (true) {
                // 遍历 CommitLog 文件,每次取出一条消息
                DispatchRequest dispatchRequest = this.checkMessageAndReturnSize(byteBuffer, checkCRCOnRecover);
                int size = dispatchRequest.getMsgSize();
                // Normal data
                // 查找结果为 true,并且消息的长度大于0
                // 表示消息正确
                ...
                // 表示已到该文件的末尾
                    ...
                    if (index >= mappedFiles.size()) {
                        ...
                        // 还有下一个文件
                        // 重置 processOffset 、mappedFileOffset
            ...
            // 更新 mappedFileQueue 的 flushedWhere 指针
            // 和 committedWhere指针
            processOffset += mappedFileOffset;
            this.mappedFileQueue.setFlushedWhere(processOffset);
            this.mappedFileQueue.setCommittedWhere(processOffset);
            // 删除 processOffset 之后所有的文件
    ...
    private static int calMsgLength(int bodyLength, int topicLength, int propertiesLength) {
        final int msgLen = 4 //TOTALSIZE 该消息条目总长度,4字节
            + 4 //MAGICCO 魔数,4字节.固定值:0xdaa320a7
            + 4 //BODYCRC 消息体crc校验码,4字节
            + 4 //QUEUEID 消息消费队列ID,4字节
            + 4 //FLAG 消息Flag,RocketMQ不做处理,供应用程序使用,4字节
            ...
            + 8 //STOREHOSTADDRESS Broker 服务器IP+端口号,8字节
    ...
    private boolean isMappedFileMatchedRecover(final MappedFile mappedFile) {
        ...
        // 判断文件的魔数,是否符合 commitlog 文件的消息存储格式
    ...
    public PutMessageResult putMessage(final MessageExtBrokerInner msg) {
        ...
        if (tranType == MessageSysFlag.TRANSACTION_NOT_TYPE
            ...
            if (msg.getDelayTimeLevel() > 0) {
                ...
                // 用延迟消息主题 SCHEDULE_TOPIC 、消息队列ID更新原消息的主题和队列
                msg.setTopic(topic);
        ...
        // mappedFileQueue可以看作是user.home/store/commitlog文件夹
        // mappedFile对应文件夹下的一个个文件
        MappedFile mappedFile = this.mappedFileQueue.getLastMappedFile();

        // 先申请写入锁(将消息存储到CommitLog文件是串行的)
        ...
        try {
            ...
            // 如果 mappedFile 为空(表明不存在任何文件,说明是第一次发送消息)
            // 或者写满
            if (null == mappedFile || mappedFile.isFull()) {
                // 用偏移量0创建第一个commit文件
                // 文件名为00000000000000000000(20位,以该文件中第一个偏移量为文件名,偏移量小于20位用0补齐)
                mappedFile = this.mappedFileQueue.getLastMappedFile(0); // Mark: NewFile may be cause noise
            }

            // 文件创建失败(可能是磁盘空间不足或者权限不够)
            if (null == mappedFile) {
    ...
    public void handleDiskFlush(AppendMessageResult result, PutMessageResult putMessageResult, MessageExt messageExt) {
        ...
        if (FlushDiskType.SYNC_FLUSH == this.defaultMessageStore.getMessageStoreConfig().getFlushDiskType()) {
            ...
            if (messageExt.isWaitStoreMsgOK()) {
                // 构建 GroupCommitRequest 同步任务
                GroupCommitRequest request = new GroupCommitRequest(result.getWroteOffset() + result.getWroteBytes());
                // 提交到 GroupCommitService
                service.putRequest(request);
                // 等待同步刷盘任务完成
                // 如果超时,则返回刷盘错误;刷盘成功后正常返回给调用方
    ...
    * */
    ...
    class FlushRealTimeService extends FlushCommitLogService {
        ...
        public void run() {
            ...
            while (!this.isStopped()) {
                ...
                // 如果距上次刷盘间隔超过 flushPhysicQueueThoroughInterval
                // 则忽略 flushPhysicQueueLeastPages 限制
            ...
            synchronized (this.requestsRead) {
                ...
                if (!this.requestsRead.isEmpty()) {
                    for (GroupCommitRequest req : this.requestsRead) {
                        ...
                        // 每执行一次刷盘操作后,立即唤醒消息发送线程,并且通知刷盘结果
                        req.wakeupCustomer(flushOK);
                    }

                    long storeTimestamp = CommitLog.this.mappedFileQueue.getStoreTimestamp();
                    if (storeTimestamp > 0) {
                        // 处理完所有同步刷盘任务后,更新 physicMsgTimestamp
                        // 但并没有执行检测点的刷盘操作,检测点的刷盘操作将在刷写消息队列文件时触发
                        CommitLog.this.defaultMessageStore.getStoreCheckpoint().setPhysicMsgTimestamp(storeTimestamp);
                    }
    ...
    class DefaultAppendMessageCallback implements AppendMessageCallback {
        ...
        public AppendMessageResult doAppend(final long fileFromOffset, final ByteBuffer byteBuffer, final int maxBlank,
            ...
            // 消息存储在 mappedFile 对应的内存映射中,并没有刷写到磁盘
